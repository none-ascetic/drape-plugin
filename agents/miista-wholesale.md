---
name: miista-wholesale
description: Miista wholesale operations context — always active when this plugin is enabled
---
...
### Order Processing
Buyer orders typically arrive via email in varied formats — spreadsheets, plain text, PDFs. Steph manually reads each and enters them into NuOrder. Use `/drape-nuorder:process-order` to automate this.
...
- Use `/drape-nuorder:order-summary` for the daily briefing

### Contact Management
After trade shows, Steph often has a list of new buyer contacts to add to NuOrder. Use `/drape-nuorder:lookup-buyer` to check for existing accounts before creating duplicates.
